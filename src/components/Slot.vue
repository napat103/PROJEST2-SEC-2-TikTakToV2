...
import { shuffle, testWin, countScore } from '../composable/function.js'
    ...
    if (score.value >= 20) {
        ...
        const Timer = setInterval(() => {
            ...
            if (countTime <= min) {
                ...
                historys.value.unshift(`${total.value}. ${slot1.value} ${slot2.value} ${slot3.value} : ${status.value}${scoreHistory}`)
                information.value.historys = historys.value
                information.value.winCount = winCount.value
                addNewSlot(information.value)
                clearInterval(Timer)
...
const runAutospin = () => {
    autoStatus.value ? stopAutospin() : Inter = setInterval(() => {
        score.value >= 20 ? doSlots() : stopAutospin();
    }, 2000)
}
const stopAutospin = () => {
...
onMounted(async () => {
    ...
    information.value = await getInformation(currentUser.value.activateUser)
...
<template>
    <Popup :score="score" :scoreToShow="scoreToShow" :status="status" @clear="clearSlots" />
    ...
    <div class="w-full h-full bg-purple-900">
        ...
        <div class="flex w-full rounded-md flex-col lg:flex-row mb-10">
            <div class="lg:w-3/5 lg:mt-10 lg:ml-10 lg:mb-0 lg:mr-0">
                <div class="w-full pb-9 lg:mb-0 mb-5 bg-purple-500 rounded border-solid border-2 text-white border-black">
                    ...
                    <div class="w-full flex justify-between">
                        <b class="pt-0 p-2 text-2xl bg-black text-white">{{ information.icon }} : {{ information.name }}</b>
                        <b :class="status === 'YOU WIN!' ? 'text-green-500' : status === 'YOU LOSE!' ? 'text-red-500' : 'text-white'"
                            class="pt-0 p-2 text-2xl bg-black">{{ status }}
                            <progress class="flex absolute pt-0 p-2 pr-5 text-sm" :value=load max="100"
                                v-show="status === 'SPINNING!'"></progress>
                        </b>
                    </div>
                    <!-- /แถบบอกสถานะซ้ายบน -->
            ...
            <div class="flex lg:flex-col md:flex-row flex-col lg:w-2/5 justify-center items-center ">
                ...
                <div
                    class="flex font-extrabold bg-black  text-3xl  p-4 mx-auto mt-10  mb-5 text-white rounded-md lg:flex-col xl:flex-row">
                ...
                <div class="flex font-extrabold justify-center">
                    <button :disabled="buttonStatus || autoStatus" @click="doSlots()"
